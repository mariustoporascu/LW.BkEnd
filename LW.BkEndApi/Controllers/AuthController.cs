...
namespace LW.BkEndApi.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> LoginAsync([FromBody] AuthModel authModel)
        {
            ...
            user.ConexiuniConturi = await _context.ConexiuniConturi
            ...
            var refreshTokenExpiry = DateTime.UtcNow.AddDays(
                int.Parse(_configuration["Jwt:ResetTokenLifeTime"])
            );
            ...
            var identityToken = new
            {
                ...
                Value = JsonConvert.SerializeObject(new { refreshToken, refreshTokenExpiry }),
            };
            // save identityToken to database
            var setAuthTokenResult = await _signInManager.UserManager.SetAuthenticationTokenAsync(
                user,
                identityToken.LoginProvider,
                identityToken.Name,
                identityToken.Value
            );
        ...
        public async Task<IActionResult> RegisterAsync([FromBody] AuthModel authModel)
        {
            ...
            var code = WebEncoders.Base64UrlEncode(
                Encoding.UTF8.GetBytes(
                    await _signInManager.UserManager.GenerateEmailConfirmationTokenAsync(user)
                )
            );
            var callbackUrl =
                $"{_configuration["Front-end-url"]}/auth/confirmation-completion?token={code}&email={user.Email}";

            var setConfirmationTokenResult =
                await _signInManager.UserManager.SetAuthenticationTokenAsync(
                    user,
                    "Emailconfirmation",
                    "Emailconfirmation",
                    code
            ...
            if (setConfirmationTokenResult.Succeeded)
            {
                if (
                    !_emailSender.SendEmail(
                        ...
                        $"Confirma-ti email-ul accesand <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>link-ul acesta</a>."
                    )
                )
                {
        ...
        public async Task<IActionResult> RefreshToken(string refreshToken, string refreshTokenId)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Unauthorized(
                    new { Message = "Refresh token or jwt token invalid", Error = true }
                );
            }
            ...
            var userToken = await _signInManager.UserManager.GetAuthenticationTokenAsync(
                user,
                "JWT",
                refreshTokenId
            );
            if (
                user == null
                || !_tokenService.ValidateRefreshToken(
                    JsonConvert.DeserializeObject<JObject>(userToken),
                    refreshToken
                )
            )
            {
        ...
        public async Task<IActionResult> PasswordResetAsync(
        ...
        )
        {
            ...
            {
                return BadRequest(
                    new { Message = "Please provide reset password token", Error = true }
                );
            }
            var user = await _signInManager.UserManager.FindByEmailAsync(authModel.Email);
            if (user == null)
            ...
            var dbToken = await _signInManager.UserManager.GetAuthenticationTokenAsync(
                user,
                "Passwordreset",
                "Passwordreset"
            );
            ...
            var resetResult = await _signInManager.UserManager.AddPasswordAsync(
                user,
                authModel.Password
            );
            if (resetResult.Succeeded)
            {
                await _signInManager.UserManager.RemoveAuthenticationTokenAsync(
                    user,
                    "Passwordreset",
                    "Passwordreset"
                );
        ...
        public async Task<IActionResult> PasswordResetTokenAsync(string email)
        {
            ...
            if (
                !string.IsNullOrWhiteSpace(
                    await _signInManager.UserManager.GetAuthenticationTokenAsync(
                        user,
                        "Passwordreset",
                        "Passwordreset"
                    )
            ...
            var callbackUrl =
                $"{_configuration["Front-end-url"]}/auth/reset-password?token={code}&email={user.Email}";
            ...
            if (setConfirmationTokenResult.Succeeded)
            {
                if (
                    _emailSender.SendEmail(
                        new string[] { email },
                        "Password reset code",
                        $"Reseteaza-ti parola accesand <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>link-ul acesta</a>."
                    )
                )
                {
            ...
            return BadRequest(
                new { Message = "Password reset token failed to get generated", Error = true }
            );
        }
        ...
        )
        {
            ...
            {
                return BadRequest(
                    new { Message = "Please provide email confirmation token", Error = true }
                );
            }
            var user = await _signInManager.UserManager.FindByEmailAsync(email);
        ...
        public async Task<IActionResult> ResendEmailConfirmationAsync(string email)
        {
            ...
            {
                return BadRequest(
                    new { Message = "User not found, or email already confirmed", Error = true }
                );
            }
            if (
                !string.IsNullOrWhiteSpace(
            ...
            return BadRequest(
                new { Message = "Email confirmation token failed to get generated", Error = true }
            );
        }
        ...
        public async Task<IActionResult> UpdateProfileAsync([FromBody] AuthModel authModel)
        {
            ...
            var conexCont = _context.ConexiuniConturi
                .Include(c => c.ProfilCont)
            ...
            conexCont.ProfilCont.PhoneNumber =
                authModel.PhoneNumber ?? conexCont.ProfilCont.PhoneNumber;
            await _signInManager.UserManager.UpdateAsync(user);
